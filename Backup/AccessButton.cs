...
*/			MenuItem end = new MenuItem();
		...
		{
			MainForm.WriteDebug("down: dip=" + dragInProgress);
			if (!dragInProgress)
			{
		...
		{
			MainForm.WriteDebug("up:start, MX="+MouseDownX+",MY="+MouseDownY+",LX="+this.Location.X+",LY="+this.Location.Y);
			...
			MainForm.WriteDebug("up:end, dip="+dragInProgress);
...
*/
		...
		private void winsTimer_Tick(object sender, EventArgs e)
		{
			...
			MainForm.WriteDebug("tick: w.count=" + wins.Count + ", f.vis=" + frontera.Visible);
			if (wins.Count == 0)
			{
				if (!frontera.Visible)
				{
