...
#region Process class
	...
	public class Process : IComparable
	{
		...
		public void Kill()
		{
			...
			hProcess = CoreDll.OpenProcess(CoreDll.PROCESS_TERMINATE, false, (int) handle);

			if(hProcess != (IntPtr)CoreDll.INVALID_HANDLE_VALUE) 
			{
				bool bRet;
				bRet = CoreDll.TerminateProcess(hProcess, 0);
				CoreDll.CloseHandle(hProcess);
			}
		}
...
#if true
			IntPtr handle = CoreDll.CreateToolhelp32Snapshot(CoreDll.TH32CS_SNAPPROCESS, 0);
			if ((int)handle > 0)
			{
				try
				{
					...
					int retval = CoreDll.Process32First(handle, peBytes);
					...
					while(retval == 1)
					{
						...
						retval = CoreDll.Process32Next(handle, peBytes);
					}
				}
				catch(Exception ex)
				{
				...
				//Close handle
				CoreDll.CloseToolhelp32Snapshot(handle); 
...
#else
			return new Process[] {new Process(IntPtr.Zero, "Unavailable", 0, 0)};
#endif
		}

    public int CompareTo(object obj)
    {
      Process Compare = (Process)obj;
      return this.ProcessName.ToUpper().CompareTo(Compare.ProcessName.ToUpper());
    }
#endregion
